...
class SplitCifar10(data.Dataset):
    ...
    def __init__(self, cfg, augmentation, train=True):
        ...
        # データセット全体を用意
        subset_indices = []
        _dataset = datasets.CIFAR10(root=self.data_folder,
                                    download=True,
                                    train=self.train)
...
def dataset_cifar10(cfg, train_aug, val_aug):
    ...
    subset_indices = []
    _train_dataset = datasets.CIFAR10(root=data_folder,
                                      transform=train_aug,
                                      download=True)
